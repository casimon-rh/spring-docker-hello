...
for color in $(echo "blue green"); do
  oc new-app openjdk-8~https://github.com/casimon-rh/spring-docker-hello#$color --name java-$color -p JAVA_APP_JAR=demo.jar
  oc rollout pause deployment/java-$color
  oc set probe deployment/java-$color --remove --readiness --liveness
  oc set probe deployment/java-$color --liveness --get-url=http://:8080/actuator/health --initial-delay-seconds=40
  oc set probe deployment/java-$color --readiness --get-url=http://:8080/actuator/health --initial-delay-seconds=40
